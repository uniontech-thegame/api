...
router.get('/', function (req, res) {
  ...
  asyncS(() => {
    try {
      const availableEnigmas = awaitS(db.manyOrNone(`
        SELECT code FROM enigmas WHERE redeem_date IS NULL
      `))

      toReturn.availableEnigmas = []
      for (let enigma of availableEnigmas) toReturn.availableEnigmas.push({ id: enigma.code })

      const availableGifts = awaitS(db.many(`
        SELECT q.location, SUM(q.count) AS count FROM (
          SELECT 0 AS count, unnest(enum_range(NULL::giftlocation)) AS location
          UNION ALL
          SELECT 1 AS count, location FROM gifts WHERE redeem_date IS NULL
        ) q GROUP BY q.location
      `))

      toReturn.availableGifts = { total: 0 }
      for (let giftLocation of availableGifts) {
        const count = parseInt(giftLocation.count, 10)
        toReturn.availableGifts[giftLocation.location.toLowerCase()] = count
        toReturn.availableGifts.total += count
      }
      ...
      for (let activityResult of activitiesResults) {
        toReturn.teams[activityResult.team_name].activitiesResults.push({
